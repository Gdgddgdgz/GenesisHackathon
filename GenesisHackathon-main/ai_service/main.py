...
def get_market_context(category):
    ...
    # 3. Upcoming Festival Scan (Next 45 days - Extended for Strategic Planning)
    for date_str, event in FESTIVAL_CALENDAR_2026.items():
        ...
        if 0 <= days_until <= 45:
            ...
            if is_relevant:
                signals.append(f"UPCOMING STRATEGIC EVENT: {event} in {days_until} days. PREPARE INVENTORY NOW.")

    if not signals:
         signals.append("SEASONAL TRANSITION: Prepare for upcoming Summer/Wedding season demand.")
...
def get_seasonal_outlook(category: str = Query("General"), lat: float = Query(None), lon: float = Query(None)):
    ...
    print(f"DEBUG: Seasonal Request - Raw: {category}, Normalized: {category}")
    ...
    forecast_start = (datetime.now() + timedelta(days=1)).strftime("%d %B")
    forecast_end = (datetime.now() + timedelta(days=30)).strftime("%d %B %Y") # Extended Horizon
    
    market_signals = get_market_context(category) # Pass category for filtering
    
    # 2. Hard Constraints Prompt
    prompt = f"""
    [CRITICAL MISSION: STRATEGIC FUTURE FORECASTING]
    ACTIVE_CATEGORY: {category}
    CURRENT_DATE: {today_date}
    STRATEGIC_PLANNING_HORIZON: {forecast_start} to {forecast_end}
    
    TASK: Generate exactly 3 forward-looking strategic predictions for the '{category}' sector based on UPCOMING FESTIVALS and SEASONAL SHIFTS.
    ...
    BEHAVIORAL GUIDANCE (STYLE & TONE):
    - **FUTURE SIMULATION**: Even if no major festival is imminent, YOU MUST SIMULATE the next likely seasonal shift (e.g., "End of Season Sale", "Early Summer Prep", "Monsoon Readiness").
    - **TITLES**: Forward-looking and Actionable (e.g., "Predicting Pre-Holi Surge", "Forecast: Ramzan Inventory Spike").
    - **INSIGHTS**: "Based on upcoming [Event/Season], we project a [Metric] increase in [Item] over the next 30 days..."
    - **SURGE METRIC**: You MUST estimate a growth percentage. Use format: "+XX% SURGE". (e.g. "+35% SURGE", "+12% GROWTH").
    - **INDIAN CONTEXT**: Use Indian terminology (e.g., "Kurta", "Kirana", "Thandai", "Gulal", "Sevaiyan").
    - **REALISM**: Focus on tangible inventory velocity.
    
    OUTPUT FORMAT (Strict JSON list of 3):
    STRICTLY return ONLY the JSON list. Do not include any preamble.
    [
      {{
        "event": "Punchy Title Here",
        "type": "{category}",
        "categories": ["{category} Item 1", "{category} Item 2"],
        "insight": "Detailed causal explanation...",
        "surge": "+XX% SURGE" 
      }}
    ]
    """
    ...
    # CRITICAL FALLBACK: Return error state instead of fake insights
    # This makes LLM failures visible to the user
    return [
        {
            ...
            "insight": f"The AI prediction engine is currently unable to generate insights for {category}. This may be due to API rate limits or connectivity issues. Please refresh or try again in a moment."
        }
    ]
